...
@Component({
    selector: 'app-proximity-glow-cards',
    standalone: true,
...
export class ProximityGlowCardsComponent {
    ...
    update(event: PointerEvent): void {
        ...
        if (
            ...
            event.x <
                card_bounds.left + card_bounds.width + this.config.proximity &&
            event.y > card_bounds.top - this.config.proximity &&
            event.y <
                card_bounds.top + card_bounds.height + this.config.proximity
        ) {
        ...
        let angle =
            (Math.atan2(event.y - card_center.y, event?.x - card_center.x) *
                180) /
            Math.PI;
